		...
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
				...
				if p.handleSession(req.Host) && pl != nil {
					...
					//log.Warning("Information inside L %s", hiblue.Sprint(l))
					//log.Warning("Information inside Pl Name %s", hiblue.Sprint(pl_name))
					//log.Warning("Information inside Req Path %s", hiblue.Sprint(req_path))
					//log.Warning("Information inside err %s", hiblue.Sprint(err))
					if err == nil {
					...
					if create_session /*&& !p.isWhitelistedIP(remote_addr, pl.Name)*/ { 
						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								log.Warning("[%s] unauthorized request: %s (%s) [%s]", hiblue.Sprint(pl_name), hiblue.Sprint(req_url), req.Header.Get("User-Agent"), remote_addr)
